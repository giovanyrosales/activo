...
if (isset($_POST["fechainicial"]) && isset($_POST["fechafinal"]) && isset($_POST["tipoBien"])) {
    ...
    header("location:Descargos.php");
} else {
    $bienes = new Bienes();
    $datostransaccion = $bienes->getDescargosByDates($_COOKIE["fechainicial"], $_COOKIE["fechafinal"], $_COOKIE["tipoBien"]);
    ...
    $title = "Descargos realizados desde " . $_COOKIE["fechainicial"] . " hasta " . $_COOKIE["fechafinal"];
 ...
 $pdf = new PDF('P', 'cm', 'A4');
    ...
    if (isset($datostransaccion) && $datostransaccion != null) {
        ...
        $pdf->Cell(3.5, $altoCabecera, "Fecha Descargo", 1, 0, 'C', 1);
        ...
        foreach ($datostransaccion as $descargo) {
            $total += $descargo['historyvalor'];
            $pdf->Cell(1, $altoCabecera, '', 0, 0, 'L');
            $pdf->SetFont('Arial', '', 9);
            $pdf->Row(array(utf8_decode($descargo['codigo']), utf8_decode($descargo['descripcion']), $descargo['fechadescargo'], $descargo['historyvalor']));
        }
