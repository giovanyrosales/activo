...
class PDF extends PDF_MC_Table
{
    ...
    public function Header(){
        ...
        $this->Image('../images/Logo2.png', 18, 1, 1.7, 2.3, 'PNG', '');
        $this->Ln(0.2);
...
if (isset($_POST["fechainicial"]) && isset($_POST["fechafinal"]) && isset($_POST["tipoBien"])) {
    ...
    header("location:Comodatos.php");
} else {
    $bienes = new Bienes();
    $datostransaccion = $bienes->getComodatosByDates($_COOKIE["fechainicial"], $_COOKIE["fechafinal"], $_COOKIE["tipoBien"]);
    ...
    $title = "Comodatos realizadas desde " . $_COOKIE["fechainicial"] . " hasta " . $_COOKIE["fechafinal"];
    ...
    if (isset($datostransaccion) && $datostransaccion != null) {
        ...
        $pdf->SetWidths(array(2, 7.5, 3.5, 2, 2));
        ...
        $pdf->Cell(7.5, $altoCabecera, "Descripcion", 1, 0, 'C', 1);
        $pdf->Cell(3.5, $altoCabecera, "Institucion", 1, 0, 'C', 1);
        $pdf->Cell(2, $altoCabecera, "Fecha", 1, 0, 'C', 1);
        $pdf->Cell(2, $altoCabecera, "Valor", 1, 1, 'C', 1);

        //Filas de los datos de la tabla de Resumen de reporte de salidas
        $total = 00.00;
        foreach ($datostransaccion as $comodato) {
            $total += $comodato['valor'];
            $pdf->Cell(1, $altoCabecera, '', 0, 0, 'L');
            $pdf->SetFont('Arial', '', 8.5);
            $pdf->Row(array(utf8_decode($comodato['codigo']), utf8_decode($comodato['descripcion']), $comodato['institucion'], $comodato['fecha'], $comodato['valor']));
        }
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(1, $altoCabecera, '', 0, 0, 'L');
        $pdf->Cell(15, $altoCabecera, 'TOTAL', 1, 0, 'C');
        $pdf->Cell(2, $altoCabecera,'$'. $total, 1, 0, 'C');
    } else {
