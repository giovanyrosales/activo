...
class PDF extends PDF_MC_Table {
    ...
    public function Header() {
        ...
        $this->Cell(7, 0.5, 'REPORTE DE REEVALUOS', 0, 0, 'C');
...
if (isset($_POST["fechainicial"]) && isset($_POST["fechafinal"]) && isset($_POST["tipoBien"])) {
    ...
    header("location:reevaluos.php");
} else {
    $bienes = new Bienes();
    $datostransaccion = $bienes->getReevaluosByDates($_COOKIE["fechainicial"], $_COOKIE["fechafinal"], $_COOKIE["tipoBien"]);
    ...
    $title = "Reevaluos realizados desde " . $_COOKIE["fechainicial"] . " hasta " . $_COOKIE["fechafinal"];
 ...
 $pdf = new PDF('P', 'cm', 'A4');
    ...
    if (isset($datostransaccion) && $datostransaccion != null) {
        ...
        $pdf->SetWidths(array(2, 7,2,3.5, 2.3));
        ...
        $pdf->Cell(7, $altoCabecera, "Descripcion", 1, 0, 'C', 1);
        $pdf->Cell(2, $altoCabecera, "Valor", 1, 0, 'C', 1);
        $pdf->Cell(3.5, $altoCabecera, "Valor Reevaluo", 1, 0, 'C', 1);
        $pdf->Cell(2.3, $altoCabecera, "Fecha", 1, 1, 'C', 1);
        
        //Filas de los datos de la tabla de Resumen de reporte de salidas
        foreach ($datostransaccion as $descargo) {
            $pdf->Cell(1, $altoCabecera, '', 0, 0, 'L');
            $pdf->SetFont('Arial', '', 9);
            $pdf->Row(array(utf8_decode($descargo['codigo']), utf8_decode($descargo['descripcion']),$descargo['valor'], $descargo['valornuevo'], $descargo['fecha'], ));
        }
    } else {
